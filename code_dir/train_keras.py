...
from helper import CreateXY, generator, read_val_data
import numpy as np
import os
import sys
import pickle
...
def read_data(fold_num):
	...
	# For ALL data
	image_paths, labels_dict, y, enc = CreateXY(data_path)
	...
	for train_index, test_index in skf.split(image_paths, y):
		...
		print('X_train size = {}, X_test size = {}'.format(
...
def load_split_dataset(fold_num):
	...
	X_train_temp, X_test, y_train_temp, y_test, enc = read_data(
		fold_num)  # choose fold 0 - 9
	print('X_test size = {}'.format(len(X_test)))

	# Now that we have train and test set, we have to make valid set
	X_train, y_train, X_valid, y_valid = create_valid_set(
		X_train_temp, y_train_temp)
	print('X_train size = {}, X_valid size = {}'.format(
